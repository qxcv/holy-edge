...
class DataParser():
    def __init__(self, cfgs):
        ...
        self.training_ids = self.all_ids[:int(
            self.cfgs['train_split'] * len(self.training_pairs))]
        self.validation_ids = self.all_ids[int(
            self.cfgs['train_split'] * len(self.training_pairs)):]

        self.io.print_info('Training samples {}'.format(
            len(self.training_ids)))
    ...
    def get_batch(self, batch):
        ...
        for idx, b in enumerate(batch):
            ...
            # Labels needs to be 1 or 0 (edge pixel or not) or can use
            # regression targets as done by the author
