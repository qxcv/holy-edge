...
def main(args):
    if not (args.run_train or args.run_test or args.download_data):
        print('Set at least one of the options --train | --test | '
              '--download-data')
        parser.print_help()
        return
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Utility for Training/Testing DL models(Concepts/'
        'Captions) using theano/keras')
    parser.add_argument(
        '--config-file',
        dest='config_file',
        default='hed/configs/hed.yaml',
        type=str,
